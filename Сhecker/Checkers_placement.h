...
private:
	/// Массив, в котором содержатся все шашки
	vector <Checker_attribute> checkers;
	/// Объект класса Board, содержащий всю информацию о положении клеток на доске
	Board board;
public:
	/// Возвращает указатель на доску
	Board &get_board() {
		return this->board;
	}
